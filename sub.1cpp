class Solution {
public:
    int minimumArea(vector<vector<int>>& grid, int st_i, int en_i, int st_j,
                    int en_j) {
        int i, j, start_row = 1e9, end_row = -1, start_col = 1e9, end_col = -1,
                  found = 0;
        for (i = st_i; i <= en_i; i++)
            for (j = st_j; j <= en_j; j++) {
                if (grid[i][j]) {
    ...
    int minimumSum(vector<vector<int>>& grid) {
        int i, j, m = grid.size(), n = grid[0].size(), ans = 1e9, one, two,
                  three;
        for (i = 0; i < m; i++) {
